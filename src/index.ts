...
server.tool(
  ...
  async ({ amount, difficulty }) => {
    ...
    const questions = data.results.map((q: any, i: number) => {
      const allAnswers = [...q.incorrect_answers, q.correct_answer].sort(() => Math.random() - 0.5);
      return [
        `**Q${i + 1}: ${decodeHTML(q.question)}**`,
        `Category: ${q.category} | Difficulty: ${q.difficulty}`,
        ...allAnswers.map((a: string, j: number) => `  ${String.fromCharCode(65 + j)}. ${decodeHTML(a)}`),
...
server.tool(
  ...
  async () => {
    ...
    const data = await res.json();
    const list = data.trivia_categories.map((c: any) => `- ${c.name} (ID: ${c.id})`).join("\n");
